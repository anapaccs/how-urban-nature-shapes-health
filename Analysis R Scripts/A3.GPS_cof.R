...
pca_lags <- c("lag13_cofPC1", "lag13_cofPC2", "lag13_cofPC3")
...
# =============================================================================
all_gps_vars <- c(confounders, pca_lags)
...
print(metrics)
write.csv(metrics,  "Results/GPS/gps_comparefit_lm_gamcof.csv", row.names = FALSE)
...
gg_raw <- df %>%
  ...
  labs(title = "Weighted GAM GPS density by treatment quintile (raw)",
       ...
       fill  = "Quintile") +
  theme_minimal()
ggsave("Results/GPS/overlap_rawcof.png", gg_raw, width = 6, height = 4)
...
gg_winsor <- df_winsor %>%
  ...
  theme_minimal()
# Save plot
ggsave("Results/GPS/overlap_winsorcof.png", gg_winsor, width = 6, height = 4)
...
gg_trim <- df_trim %>%
  ...
  theme_minimal()
# Save plot
ggsave("Results/GPS/overlap_trimcof.png", gg_trim, width = 6, height = 4)
...
# Save plot
ggsave("Results/GPS/hist_sw_rawcof.png", gg_hist_raw, width = 6, height = 4)
...
# Save plot
ggsave("Results/GPS/hist_sw_trimcof.png", gg_hist_trim, width = 6, height = 4)
...
# Save plot
ggsave("Results/GPS/hist_sw_wincof.png", gg_hist_win, width = 6, height = 4)
...
# 9a) Data
write_rds(df,       "Results/GPS/gps_weightscof.rds")
write_rds(df_trim,  "Results/GPS/gps_trimmedcof.rds")
write_rds(df_winsor,"Results/GPS/gps_winsorizedcof.rds")

# 9b) Models & metrics
write_rds(lm_gps,  "Results/GPS/lm_gpscof.rds")
write_rds(gam_gps, "Results/GPS/gam_gpscof.rds")

# 9c) Balance tables
df_bal_unw <- as.data.frame(bal.tab(treatment ~ ., data = df, weights = rep(1,nrow(df)))$Balance)
write_csv(df_bal_unw, "Results/GPS/balance_unweightedcof.csv")
df_bal_trim <- as.data.frame(bal.tab(treatment ~ ., data = df_trim, weights = df_trim$sw_trim)$Balance)
write_csv(df_bal_trim, "Results/GPS/balance_trimmedcof.csv")
df_bal_win  <- as.data.frame(bal.tab(treatment ~ ., data = df_winsor, weights = df_winsor$sw_win)$Balance)
write_csv(df_bal_win,  "Results/GPS/balance_winsorizedcof.csv")
